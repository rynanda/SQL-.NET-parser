...
and performWhere (condition:Expr option) (table:Relation) : Relation =
    ...
    | Some expr ->
        ...
        // Use a mutable row array state - arrays are mutable while lists are not
        let mutable mutateRows = Array.empty
        ...
        for row in table.rows do
            ...
            | SQLIntValue 1 ->
                // If the row matches the expression, append to mutateRows array
                mutateRows <- Array.append mutateRows [|row|]
            | _ -> () // If not, do nothing
        
        // Return new relation with mutated rows
        { table with rows = Seq.ofArray mutateRows}
...
and performGroupBy (groupbyColumns: Expr list option) (table:Relation): Relation =
    ...
    | Some columns ->
        ...
        let groupRows : Row seq =
            table.rows
            |> Seq.groupBy groupingKey // Group the rows by the grouping key
            |> Seq.map (fun (key, rows) -> rows) // Get the grouped rows
            |> Seq.concat // Concatenate the grouped rows into a single sequence (as opposed to Row seq seq)
        ...
        { columnsInfo = newColumnInfos; rows = groupRows; groups = None}
